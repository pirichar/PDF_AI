...
export default async function Pricing(){
	...
	const user = authCheck.isAuthentificated ? await currentUser() : null;

	// Get Subscription data if authenticated
	const subscrition = authCheck.isAuthentificated ? await getData(authCheck.userId) : null;

	const isSubscribed = subscrition?.status === "active";
	...
	async function createSubscription(){
		...
		if (!databaseUser){
			throw new Error('DatabasxeUser Not Found')
		}
	...
	async function createCustomerPortal(){
		"use server"
		if (!authCheck.userId){
			return redirect('sign-in?redirect_url=/princing')
		}

		const customerPortalUrl = await stripe.billingPortal.sessions.create({
			customer: subscrition?.user.stripeCustomerId as string,
	...
	const backLink = authCheck.isAuthentificated ? '/dashboard' : '/';
	...
	return (
		<div className="py-16 px-4"> 
			<div className="max-w-4xl mx-auto">
				<Link
					href={backLink}
					className="text-white/70 hover:text-white inline-flex items-center mb-8 transition-all duraiton-300
					...
					<div className="space-y-6">
						...
						<div className="space-y-4">
							<p className="text-white/80 text-lg">
								Access top all features
							</p>
							<p className="text-3lx font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-purple-200">
								19.99/year
							</p>
